...
对中间件或应用程序本身进行显式的、基于标注的插桩，或许是分布式系统监控中更为常见的一种方法。Pip 和 Webmon 是更依赖应用层标注的系统代表，而 X-Trace、Pinpoint 以及早期版本的 Magpie 则主要侧重于对库和中间件进行修改。Dapper 与后者这一类系统的关系最为密切。与 Pinpoint、X-Trace 以及早期 Magpie 类似，Dapper 使用一个全局标识符，将分布式系统不同部分中相互关联的事件串联起来。也同样类似于这些系统，Dapper 试图通过把插桩隐藏在通用软件模块内部，来避免必须对应用程序本身进行标注。Magpie 则放弃了使用全局 ID 及其正确传播所带来的挑战，转而采用一种事件模式（event schema）：这种模式需要为每个应用单独编写，并显式描述事件之间的关系。对我们而言，这类 schema 在实践中究竟能在多大程度上实现透明性，仍不十分明确。X-Trace 对核心标注的要求比 Dapper 更为激进一些：它不仅在节点边界收集追踪信息，还会在控制权于同一节点内部不同软件层之间传递时进行追踪。由于我们对低开销插桩有着严格要求，因此没有采用这种模型，而是转向构建一组尽可能精简的机制，以便把所有代表某个原始请求完成的工作关联起来。当然，Dapper 的追踪结果仍然可以通过可选的应用层标注进一步丰富。
