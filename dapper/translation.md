...
本文介绍了 Google 在线上生产环境中使用的分布式系统追踪基础设施—— **Dapper**，并说明它是如何实现**三项**核心设计目标：**低开销**、**对应用层透明**，以及能够在超大规模系统中广泛部署。Dapper 与其他链路追踪系统有不少相似之处，尤其是 Magpie 和 X-Trace。但Dapper在一些关键设计取舍上做出了更适合 Google 运行环境的选择，而这些选择也正是它成功的重要原因。比如，它采用了**采样机制**，并且将埋点工作尽量限制在少量通用库中，从而有效控制了系统复杂度和接入成本。
...
![[]()](../pic/dapper1.png){:width="100px" height="100px"}
<center>图 1</center>
...
为总结这些信息，从而将所有记录条目与某个给定的发起者（例如图 1 中的 RequestX）关联起来，我们在研究中提出了两类方案：**黑盒监测方案**和**基于标注的监测方案**。黑盒方案是基于假定除前文所述的消息记录之外，不存在任何额外信息，并利用统计回归技术来推断这种关联关系。基于标注的方案则依赖应用程序或中间件，显式地为每条记录打上一个**全局标识符**，以便将这些消息记录回溯并关联到原始请求。尽管黑盒方案比基于标注的方法具有更好的**可移植性**，但由于其**依赖统计推断**，因此为了获得足够的准确性，需要更多的数据。相比之下，基于标注方法的关键缺点显然在于必须对程序进行**插桩**。在我们的环境中，由于所有应用都采用相同的线程模型、控制流和 RPC 系统，我们发现可以将插桩限制在一小部分公共库中，从而实现一种对应用开发者实际上近乎透明的监测系统。
...
在 **Dapper 追踪树（Trace trees）** 中，树的节点是工作的基本单元，我们称之为**跨度（span）**。边则表示某个 span 与其**父跨度（parent span）**之间的因果关系。不过，抛开它在更大追踪树中的位置不谈，一个 span 本身也可以看作是一个简单的、带时间戳的记录日志，用于编码该 span 的开始时间与结束时间、任意 RPC 请求时序数据，以及多个应用特定的标注；关于后者，将在第 2.3 节中讨论。
...
+ 当某个线程处理一条被追踪的控制路径时，Dapper 会将一个 **追踪上下文（trace context）** 附加到线程的**局部存储（thread-local storage）**中。追踪上下文是一个小型且易于复制的容器，其中保存了诸如 **trace id** 和 **span id** 之类的 span 属性。
...
+ Google 几乎所有的**进程间通信（IPC）**都构建在同一个 **RPC 框架**上。我们对该框架进行了插桩，使其能够围绕所有 RPC 定义 **span**。对于被追踪的 RPC，**span id** 和 **trace id** 会从客户端传递到服务端。对于像 Google 内部这样被广泛使用的基于 RPC 的系统而言，这是一个至关重要的插桩点。随着非 RPC 通信框架的不断演进并逐步形成用户基础，我们也计划对其进行插桩。
...
除简单的文本标注之外，Dapper 还支持一种**键值对标注映射（map of key-value annotations）**，从而为开发者提供更强的跟踪能力，例如维护计数器、记录二进制消息，以及在进程内部沿着被跟踪请求传递任意用户定义数据。

### 2.4 Sampling
**低开销**是 Dapper 的一个核心设计目标，因为如果一个新工具价值尚未得到证明，却会对性能产生任何显著影响，那么服务运维人员显然不愿意部署它。此外，我们也希望开发者能够在不担心额外开销的前提下使用标注 API。我们还发现，某些类型的 Web 服务确实对插桩开销较为敏感。因此，除了尽可能降低 Dapper 采集过程中的基础插桩开销之外，我们还通过只记录全部跟踪中的一部分来进一步控制开销。关于这种**跟踪采样方案**，我们将在第 4.4 节中作更详细的讨论。

### 2.5 Trace collection
Dapper 的**跟踪日志记录与采集流水线**是一个三阶段过程（见图 5）。首先，span 数据会被写入（1）本地日志文件；随后，Dapper 守护进程和采集基础设施会从所有生产主机上将其拉取出来（2）；最后，再将这些数据写入（3）若干区域性 Dapper Bigtable 仓库中的某个单元（cell）。一条 trace 在 Bigtable 中被列为单独的一行，其中每一列对应一个 span。这里，Bigtable 对稀疏表的布局非常有用，因为单条 trace 所包含的 span 数量可以是任意的。trace 数据采集的中位延迟，即也就是数据从经过插桩的应用二进制程序传播到中央仓库所需的时间小于 15 秒。p98 延迟随时间呈现双峰分布：大约在 75% 的时间里，p98 的采集延迟低于 2 分钟；而在其余约 25% 的时间里，该延迟可能增长到数小时。
...
如前文所述，Dapper 系统的跟踪日志记录与采集是相对于请求树本身**带外（out-of-band）**进行的。之所以这样设计，有两个彼此独立的原因。
...
2. **带内（in-band）采集方案**默认假设所有 RPC 都是严格嵌套的。但我们发现，许多中间件系统会在其自身所有后端都尚未返回最终结果之前，就先向调用方返回结果。对于这种非嵌套的分布式执行模式，带内采集系统无法正确处理。
...
Dapper 代码库中也许最关键的部分，是对基础 RPC、线程以及控制流库所进行的**插桩**。这部分功能包括 span 的创建、采样以及向本地磁盘写日志。除开销要低之外，这些代码还必须保持稳定且健壮，因为它们会被链接到大量应用之中，这也使得维护和缺陷修复变得困难。核心插桩代码在 C++ 中不足 1000 行，在 Java 中不足 800 行。键值对标注功能的实现则额外增加了约 500 行代码。
...
本节将介绍 Dapper 主要插桩操作的开销、跟踪采集的开销，以及 Dapper 对生产工作负载的影响。我们还将说明 Dapper 的**自适应跟踪采样机制**如何帮助我们在低开销需求与获得有价值的跟踪之间取得平衡。

### 4.1 Trace generation overhead
跟踪生成开销是 Dapper 性能开销中最关键的部分，因为在紧急情况下，采集与分析功能更容易被关闭。Dapper 运行时库中，跟踪生成开销最主要的来源包括：创建和销毁 span 与标注，以及将其记录到本地磁盘以供后续采集。**根 span** 的创建与销毁平均耗时为 204 纳秒，而对**非根 span**执行相同操作的平均耗时为 176 纳秒。二者之间的差异，来自于为根 span 分配全局唯一 **trace id** 所增加的额外成本。
...
我们正在部署一种**自适应采样方案**。该方案不再是统一的采样概率，而是单位时间内期望采样到的 trace 数量。这样一来，低流量工作负载会自动提高其采样率，而超高流量工作负载则会相应降低采样率，从而使开销保持在可控范围内。实际采用的采样概率会与 trace 本身一同记录下来；这有助于围绕 Dapper 数据构建的分析工具准确统计 trace 的出现频率。
...
上述采样机制的设计目标，是将集成了 Dapper 运行时库的应用中可感知的开销降到最低。不过，Dapper 团队还需要控制写入中央仓库的数据总量，因此我们为此又引入了**第二轮采样**。
...
为了在实际资源需求和 Bigtable 累计写入吞吐量这两方面都保持足够的灵活性，我们在采集系统中加入了额外采样的支持。对于某一给定 trace，其所有 span（尽管它们可能分布在成千上万台不同的主机上）都共享同一个 trace id。因此，我们可以通过对 trace id 进行采样来控制跟踪数据的总量。对于采集系统中看到的每个 span，我们都会将其对应的 **trace id** 哈希为一个标量 𝑧，其中 0 ≤ 𝑧 ≤ 1。如果 𝑧 小于我们的**采集采样系数**，就保留该 span 并将其写入 Bigtable；否则便将其丢弃。由于采样决策依赖于 trace id，我们采样或丢弃的是整条 trace，而不是 trace 中的单个 span。我们发现，这一额外的配置参数使得采集流水线的管理简单了许多，因为只需修改配置文件中的一个参数，我们就能方便地调节全局写入速率。
...
Ads Review 团队广泛使用了 Dapper 的**注解 API**。借助开源 AOP 框架 Guice，团队将关键软件组件标注为“@Traced”。与此同时，追踪中还补充记录了重要子程序输入输出规模、状态消息以及其他调试信息，而这些信息在传统做法下通常会被写入日志文件。
...
一位从事长尾延迟调试的工程师开发了一个小型库，用于从 DAPI Trace 对象中推断层次化**关键路径**。随后，团队利用这些**关键路径**结构进行问题诊断，并为通用搜索中潜在的性能优化措施设定优先级。基于 Dapper 的这项工作带来了如下发现：
...
Google 的许多存储系统由多层彼此独立且各自复杂的分布式基础设施共同构成。例如，Google App Engine 建立在一个可扩展的物理存储系统之上，而该物理存储系统又在底层 BigTable 之上提供了部分关系型数据库管理系统（RDBMS）功能。Bigtable 同时依赖于 Chubby（一种分布式锁系统）和 GFS。与此同时，像 Bigtable 这样的系统通常被作为共享服务统一管理，以简化部署并提升计算资源利用率。

在这种分层体系中，终端用户的资源消耗模式并不总是容易分辨。比如，某个 BigTable 单元上较高的 GFS 流量，可能主要来源于单一用户，也可能来自多个用户。但在 GFS 这一层级上，这两种不同的使用模式往往难以区分。此外，若缺乏 Dapper 这类工具，对于此类共享服务上的资源竞争问题，调试工作同样会十分棘手。

第 5.2 节所示的 Dapper 用户界面能够跨任意共享服务的多个客户端，对追踪性能信息进行分组和聚合。这使得共享服务的管理者可以基于多种指标对用户进行排序，例如入网络负载、出网络负载，以及处理请求所消耗的总时间。

### 6.6 Firefighting with Dapper
Dapper 对部分而非全部应急排障任务都具有帮助。这里的 **`firefighting`** 指的是针对处于危急状态的分布式系统所开展的紧急处置工作。一般来说，执行此类应急排障任务的 Dapper 用户需要获取最新数据，既没有时间编写新的 DAPI 代码，也无法等待周期性报告完成。

对于那些在正常工作负载下就已表现出高延迟，甚至更严重地发生超时的服务，Dapper 的用户界面通常能够定位延迟瓶颈所在的位置。通过直接与 Dapper 守护进程通信，可以较为容易地采集到与特定高延迟追踪相关的最新数据。在发生灾难性故障时，通常没有必要查看聚合统计信息来确定根因，许多具有代表性的示例追踪往往已经足够。

然而，像第 6.5 节所描述的那类共享存储服务，在用户活动突然激增时，需要尽可能快地获得聚合信息。对于事后事件复盘，共享服务仍然可以利用 Dapper 的聚合数据；但是，如果对已收集的 Dapper 数据的批量分析无法在事件发生后 10 分钟内完成，那么 Dapper 在处理共享存储服务故障时的实际作用，就无法达到其本应具备的水平。

## 7. Other Lessons Learned
尽管迄今为止我们使用 Dapper 的经验总体上符合预期，但其中仍有一些积极的方面是我们事先未能充分预料到的。我们尤其满意的是，Dapper 催生了大量非预期的应用场景。除了第 6 节中描述的若干使用经验之外，这些场景还包括：资源核算系统、用于检查敏感服务是否遵循既定通信模式的工具，以及对 RPC 压缩策略的分析等。我们认为，这些非预期用途的出现，部分应归因于我们通过一个简单的编程接口向开发者开放了追踪数据存储；这一做法使我们能够调动一个规模更大社群的创造力。与此同时，事实也表明，为遗留工作负载添加 Dapper 支持比预想中更为简单：对于那些使用了通用线程、控制流和 RPC 框架的程序而言，通常只需使用现有库的新版本重新编译即可。

Dapper 在 Google 内部的广泛使用，也使我们获得了关于其若干局限性的宝贵反馈。下面我们将介绍迄今为止识别出的其中一些最重要的问题。

**合并效应（Coalescing effects）：** 我们的模型假设，各个子系统在任一时刻只会为一个被追踪的请求执行工作。而在某些情况下，先缓冲许多个请求，再一次性对这一组请求执行操作，会更为高效（磁盘写入合并就是一个典型例子）。在这类场景下，一个被追踪的请求可能会被错误地归因为一个表面上异常庞大的工作单元。进一步地，如果多个被追踪的请求被批量合并处理，由于我们依赖于“每条追踪对应一个唯一 trace id”的机制，这批请求中最终只有一个会在 span 归因上看起来对该段工作负责。我们正在考虑相应的解决方案，以识别这类情况，并仅记录能够消除歧义所必需的最少信息。

**批处理工作负载的追踪（Tracing batch workloads）：** Dapper 最初的设计目标是面向在线服务系统，其原始目标是理解由用户向 Google 发起请求所引发的系统行为。然而，离线的数据密集型工作负载，例如符合 MapReduce 模型的工作负载，同样能够从更深入的性能洞察中受益。在这类情况下，我们需要将一个 trace id 与某种其他有意义的工作单元关联起来，例如输入数据中的某个键（或某一段键范围），或者某个 MapReduce 分片。

**定位根因（Finding a root cause）：** Dapper 在判断系统中哪一部分发生了性能变慢方面是有效的，但并不总是足以找出问题的根本原因。举例来说，一个请求之所以变慢，未必是因为它自身的行为，而可能是因为在它之前已有其他请求排队等待处理。程序可以借助应用层标注，将队列长度或过载状态传递给追踪系统。此外，如果这类现象较为常见，那么 ProfileMe 中提出的成对采样（paired sampling）技术可能会很有帮助。其做法是对两个在时间上重叠的请求进行采样，并观察它们在整个系统中的相对延迟表现。

**记录内核级信息（Logging kernel-level information）：** 关于内核可见事件的详细信息，有时会对根因定位有所帮助。我们已有许多工具能够对内核执行过程进行追踪或其他形式的性能剖析，但要以一种通用且尽量不具侵入性的方式，将这类信息关联到驻留在用户态的追踪上下文上，仍然较难实现。我们正在研究一种折中方案，即从用户态对少量内核级活动参数进行快照采集，并将其与当前活动 span 关联起来。

## 8. Related Work
在分布式系统追踪这一领域，已经积累了相当丰富的研究成果。其中，一些系统主要聚焦于故障定位，另一些则着力于性能优化。Dapper 也曾被用于故障发现，但总体而言，它在揭示性能问题以及加深人们对大型复杂工作负载行为的整体理解方面，更加有用。

Dapper 与黑盒监控系统有一定关联，例如 Project5、WAP5 和 Sherlock 系统。这类系统由于不依赖运行时库插桩，因此可以说能够实现更高程度的应用层透明性。不过，黑盒方案的缺点在于，它们通常存在一定程度的不精确性，而且在对因果路径进行统计推断时，往往还会带来更大的系统开销。

对中间件或应用程序本身进行显式的、基于标注的插桩，或许是分布式系统监控中更为常见的一种方法。Pip 和 Webmon 是更依赖应用层标注的系统代表，而 X-Trace、Pinpoint 以及早期版本的 Magpie 则主要侧重于对库和中间件进行修改。Dapper 与后者这一类系统的关系最为密切。与 Pinpoint、X-Trace 以及早期 Magpie 类似，Dapper 使用一个**全局标识符**，将分布式系统不同部分中相互关联的事件串联起来。也同样类似于这些系统，Dapper 试图通过把插桩隐藏在通用软件模块内部，来避免必须对应用程序本身进行标注。Magpie 则放弃了使用全局 ID 及其正确传播所带来的挑战，转而采用一种**事件模式（event schema）**：这种模式需要为每个应用单独编写，并显式描述事件之间的关系。对我们而言，这类 schema 在实践中究竟能在多大程度上实现透明性，仍不十分明确。X-Trace 对核心标注的要求比 Dapper 更为激进一些：它不仅在节点边界收集追踪信息，还会在控制权于同一节点内部不同软件层之间传递时进行追踪。由于我们对低开销插桩有着严格要求，因此没有采用这种模型，而是转向构建一组尽可能精简的机制，以便把所有代表某个原始请求完成的工作关联起来。当然，Dapper 的追踪结果仍然可以通过可选的应用层标注进一步丰富。

## 9. Conclusions
本文介绍了 Dapper，Google 在生产环境中使用的分布式系统追踪平台，并汇报了我们在其开发与使用过程中的实践经验。Dapper 已部署到几乎所有 Google 系统中，使我们绝大多数大型工作负载都能够在无需任何应用层修改且几乎不带来可感知性能影响的情况下实现追踪。Dapper 对开发人员和运维团队的价值，一方面体现在其主追踪用户界面的广泛使用上，另一方面也体现在本文展示的诸多应用案例中，其中甚至包括一些连其设计者最初都未曾预料到的用途。

据我们所知，这是首篇针对大规模生产级分布式系统追踪框架进行系统性报告的文章。事实上本文是对一个已经稳定运行两年多的系统所做的回顾性总结。例如，我们发现，将一种最小化、对应用透明的追踪能力与一个供程序员增强追踪信息的简洁 API 结合起来，是非常值得的设计选择。

我们认为，Dapper 实现了比以往基于标注的分布式追踪系统更高程度的应用层透明性，这一点可以从仅有极少数工作负载需要人工干预这一事实中得到体现。尽管这一点在一定程度上得益于我们计算部署环境中某种不同寻常的一致性，但这仍然是一项相当重大的挑战。更重要的是，我们的设计表明，实现应用层透明性需要满足若干充分条件；我们希望这些经验能够帮助他人在更加异构的环境中开发相应的解决方案。

最后，通过向内部开发者开放 Dapper 的追踪数据存储，我们使得人们能够构建出远多于仅靠 Dapper 团队单独工作所能产出的分析工具，从而极大放大了设计与实现工作的整体效益。
