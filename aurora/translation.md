...
**关键词：** Databases · Distributed Systems · Log Processing · Quorum Models · Replication · Recovery · Performance · OLTP
...
在云原生环境下，传统数据库系统所面临的 **I/O 瓶颈**发生了本质上的改变：

- I/O 操作可以被分散到由大量节点和磁盘构成的多租户集群（Fleet）中，单一磁盘或节点已不再是性能热点（Hotspot）。
- 真正的瓶颈转移到了发起 I/O 请求的**数据库层**与执行 I/O 的**存储层**之间的网络连接上。
- 除了每秒包转发率（PPS）和带宽等基础瓶颈外，大规模并行写入还会进一步导致**流量放大（Amplification of traffic）**效应。
- 在这种架构下，即便整体性能卓越，个别**离群（Outlier）**的存储节点、磁盘或网络路径的性能表现，也往往会决定最终的系统响应时间。

尽管数据库中的大多数操作可以实现**并发执行**（overlap），但在某些特定场景下，**同步操作**（synchronous operations）仍是必不可少的。这些同步操作会导致系统出现**执行停顿**（stalls）以及频繁的**上下文切换**。

一个典型场景是**缓冲池未命中**：

- 由于数据库**缓冲池**（buffer cache）未命中而触发磁盘读取请求时，读取线程在数据读取完成前无法继续运行，从而进入阻塞状态。
- 缓存未命中还可能带来额外的性能开销（penalty）：为了给新调入的页面腾出空间，系统必须**驱逐**（evicting）并**刷写**（flushing）现有的“脏”缓存页。
...
<p align="center"><em>图 1：Move logging and storage off the database engine</em></p>
...
2. **利用智能存储实现架构下推**：介绍如何通过将传统数据库的**底层组件（lower quarter）**卸载（offloading）至存储层，从而充分发挥**智能存储**的性能优势。（第 3 节）
...
多副本机制进一步放大了原本就已高企的 I/O 流量，并带来两类直接负担：

- **每秒数据包数（PPS）负担**：复制放大使网络请求数快速上升。
- **同步点（Points of Synchronization）负担**：这些 I/O 操作不仅会导致执行流水线停顿（Stall），还会显著拉长延迟。

尽管链式复制（Chain Replication）及其替代方案能够有效降低网络开销，但它们依然无法摆脱**同步停顿**和**累加延迟**的困扰。
...
<p align="center"><em>图 2：Network IO in mirrored MySQL</em></p>
...
首先，在该流程中，步骤 1、3 和 5 具有明显的串行性与同步性，问题集中体现在以下方面：

- **延迟累加**：大量写入操作必须按序执行，导致延迟呈现出**累加效应（Additive Latency）**。
- **抖动放大**：即便在异步写入场景下，系统也必须等待执行最慢的操作完成才能返回，这使得**性能抖动（Jitter）**被显著放大，系统极易受“离群值”（Outliers，即慢节点请求）影响而陷入瘫痪。
- **Quorum 脆弱性**：从分布式系统的视角审视，该模型实质上可被视为执行了 **4/4 写 Quorum** 机制，因此在面对节点失效或长尾性能波动时表现得异常脆弱。
- **信息重复写入**：由 OLTP 应用引发的用户操作会产生多种不同类型的写入，而这些写入往往是对相同信息的重复表达。例如，为了防止存储基础设施层出现“页面断裂”（Torn Pages）而引入的**双写缓冲（Double Write Buffer）**操作，本质上就是对同一数据的冗余写入。
...
<p align="center"><em>图 3：Network IO in Amazon Aurora</em></p>

在 Aurora 架构中，写入路径被重新组织为以下几个关键点：

- **跨网络传输的唯一写入操作是重做日志记录（Redo Log Records）**。
- **数据库层（计算层）永远不会向外写出数据页**：无论是后台写入、**检查点（Checkpointing）**操作，还是因**缓存淘汰（Cache Eviction）**引发的写盘。
- **日志应用逻辑（Log Applicator）被下推（Pushed）到存储层**：系统可以在存储层利用这些日志，在后台或根据即时需求生成数据库页。
- **页面物化（Materialization）在后台持续执行**：如果每次生成页面都要从其诞生之初的完整修改链开始溯源，代价将高昂到难以接受；因此后台物化可以避免每次按需读取时都必须从头开始重新生成。
...
<p align="center"><em>表 1：Network IOs for Aurora vs MySQL</em></p>
...
<p align="center"><em>图 4：IO Traffic in Aurora Storage Nodes</em></p>
...
数据库中可能存在多个**执行中（Outstanding）**的隔离事务，这些事务完成（即达到终态并实现持久化）的顺序可能与其初始发起的顺序不一致。假定数据库发生崩溃或重启，针对这些独立事务是否需要执行**回滚（Rollback）**的判定是相互分离的。数据库引擎负责维护跟踪“部分完成事务”并执行**撤销（Undo）操作**的逻辑，其行为表现与向常规物理磁盘写入数据时并无二致。然而，在系统重启后、数据库获准访问存储卷之前，存储服务会执行一套自身的恢复流程。该流程的重心并非用户级事务，而是旨在确保存储层即便具备分布式特性，仍能向数据库呈现一个**统一的存储视图（Uniform View）**。

存储服务通过三个关键 LSN 来界定恢复边界：

- **VCL（Volume Complete LSN，卷完整 LSN）**：存储服务确定的最高 LSN，并保证在该序列号之前的所有日志记录均已完整可用。在存储恢复阶段，所有 LSN 大于 VCL 的日志记录都必须被截断（Truncated）。
- **CPL（Consistency Point LSN，一致性点 LSN）**：数据库通过标记特定日志记录来指定的一致性点，用于进一步限定允许进行截断的子集点。
- **VDL（Volume Durable LSN，卷持久 LSN）**：定义为“不大于 VCL 的最高 CPL”。系统会截断所有 LSN 大于 VDL 的日志记录。
...
<p align="center"><em>图 5：Aurora Architecture: A Bird's Eye View</em></p>

Aurora 的数据库引擎是社区版 MySQL/InnoDB 的一个分支（Fork），其主要差异体现在 **InnoDB 读写磁盘数据的方式**上。在社区版 InnoDB 中，写入操作会导致缓冲池页面（Buffer Pages）被修改，而相关的重做日志（Redo Log）记录会按 LSN 顺序写入预写日志（WAL）的缓冲区。在事务提交时，WAL 协议仅要求将该事务的重做日志记录持久化写入磁盘。至于实际被修改的缓冲池页面，最终会通过**双写技术（Double-write technique）**写入磁盘，以规避“部分页面写入”（即页面断裂）的问题。
...
<p align="center"><em>图 6：Aurora scales linearly for read-only workload</em></p>

![](../pic/aurora7.png)

<p align="center"><em>图 7：Aurora scales linearly for write-only workload</em></p>
...
<p align="center"><em>表 2：SysBench Write-Only (writes/sec)</em></p>
...
<p align="center"><em>表 3：SysBench OLTP (writes/sec)</em></p>
...
<p align="center"><em>表 4：Replica Lag for SysBench Write-Only (msec)</em></p>
...
<p align="center"><em>表 5：Percona TPC-C Variant (tpmC)</em></p>
...
<p align="center"><em>图 8：Web application response time</em></p>
...
<p align="center"><em>图 9：SELECT latency (P50 vs P95)</em></p>
...
<p align="center"><em>图 10：INSERT per-record latency (P50 vs P95)</em></p>
...
<p align="center"><em>图 11：Maximum Replica Lag (averaged hourly)</em></p>
...
<p align="center"><em>图 12：Zero-Downtime Patching</em></p>
